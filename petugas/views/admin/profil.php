    ...
    <form action="admin.php" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="col-span-1">
            <div class="bg-zinc-900/50 backdrop-blur-md border border-zinc-700/50 rounded-xl p-6 shadow-2xl flex flex-col items-center text-center relative overflow-hidden">
                ...
                <div class="relative z-10 w-28 h-28 rounded-full border-4 border-amber-500/40 shadow-xl mt-4 mb-2 overflow-hidden bg-zinc-800 group">
                    <?php 
                    $profile_url = get_user_avatar_url($_SESSION['user_id'], $current_user['fullname'] ?? $current_user['username'], '../');
                    ?>
                    <img id="admin_avatar_preview" src="<?= $profile_url ?>" alt="Avatar" class="w-full h-full object-cover">
                    ...
                    <input type="file" name="foto_profil" id="foto_profil_input" class="hidden" accept="image/jpeg,image/png,image/webp,image/jpg" onchange="handleProfilePhotoChange(this, 'admin_avatar_preview')">
                </div>
                ...
                <div id="admin_photo_selected_badge" class="hidden relative z-10 text-[11px] text-emerald-400 font-medium mb-3">
...
function handleProfilePhotoChange(input, previewId) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            const badge = document.getElementById('admin_photo_selected_badge');
